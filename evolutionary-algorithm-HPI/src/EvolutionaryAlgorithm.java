...
public class EvolutionaryAlgorithm {
    ...
    @Getter
    private Individual highPotentialIndividual;
    private Individual injectedHighPotential = null;
    ...
    private final double DELTA = 0.03; // Threshold for convergence
    ...
    public EvolutionaryAlgorithm(int populationSize, int generations, double mutationRate,
                                 double recombinationRate, double migrationRate,
                                 Main.ProblemInstance problemInstance,
                                 Individual injectedHighPotential) {
        this(populationSize, generations, mutationRate, recombinationRate, migrationRate, problemInstance);
        this.injectedHighPotential = injectedHighPotential;
    }

    public RunResult runEvolutionaryAlgorithmLoop() {
        Population firstGeneration;
        if (injectedHighPotential != null) {
            firstGeneration = new Population(POPULATION_SIZE, problemInstance, injectedHighPotential);
        } else {
            firstGeneration = new Population(POPULATION_SIZE, problemInstance);
        }
    ...
    public void trackHighPotentialIndividual() {
        int midGeneration = GENERATIONS / 2;

        if (bestIndividualsPerGeneration.isEmpty()) {
            System.err.println("Warning: No best individuals logged. Can't determine High Potential Individual.");
            return;
        }

        if (midGeneration < bestIndividualsPerGeneration.size()) {
            this.highPotentialIndividual = bestIndividualsPerGeneration.get(midGeneration);
        } else {
            this.highPotentialIndividual = bestIndividualsPerGeneration.get(bestIndividualsPerGeneration.size() - 1);
        }

        // 🐞 Optional debug output
        System.out.println("\n=== High Potential Individual ===");
        System.out.printf("Selected from Generation %d: [x1=%.4f, x2=%.4f] -> Fitness: %.6f\n",
                midGeneration,
                highPotentialIndividual.getX1(),
                highPotentialIndividual.getX2(),
                highPotentialIndividual.getTargetValue());
    }
}